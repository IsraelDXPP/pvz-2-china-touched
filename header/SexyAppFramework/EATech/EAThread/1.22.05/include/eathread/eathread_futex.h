...
//
    ...
    #elif defined(EA_PLATFORM_PS3)                  // PS3 has lwmutex, which is a light-weight mutex.
        ...
        #define EATHREAD_MANUAL_FUTEX_ENABLED 1     // Enable the default Futex implementation on CTR.
...
/// 
    ...
    // We avoid #including heavy system headers, as this file is a common header itself.
        ...
        #if !defined(DWORD)
            typedef unsigned long DWORD; //Have to define DWORD since we aren't including heavy system headers
        #endif
        extern "C" 
        {
            struct _RTL_CRITICAL_SECTION;

            void          __stdcall RtlInitializeCriticalSectionAndSpinCount(_RTL_CRITICAL_SECTION* pCriticalSection, DWORD dwSpinCount);
...
namespace EA
{
    namespace Thread
    {
            ...
            inline void Futex::SetSpinCount(Uint spinCount)
            {
                ...
                inline Futex::Futex()
                {
                    // To consider: Should we call RtlInitializeCriticalSectionAndSpinCount instead, with EATHREAD_FUTEX_SPIN_COUNT?
