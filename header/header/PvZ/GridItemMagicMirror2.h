...
#ifndef PLANTSVERSUSZOMBIES2_PVZ2_GRIDITMEMAGICMIRROR2_H_
#define PLANTSVERSUSZOMBIES2_PVZ2_GRIDITMEMAGICMIRROR2_H_
...
class GridItemMagicMirrorProps2 : public GridItemAnimationProps
{
public:
	RT_CLASS_DEFINE(GridItemMagicMirrorProps2, GridItemAnimationProps, RtClass);

	std::vector<std::string> ZombieTypeNameCannotBeTeleported;
	std::vector<std::string> ProjectileClassNameCannotBeTeleported;
};

class GridItemMagicMirror2: public GridItemAnimation
{
public:
	RT_CLASS_DEFINE(GridItemMagicMirror2, GridItemAnimation, RtClass);

	GridItemMagicMirror2();
	...
	void setBrotherMagicMirror(GridItemMagicMirror2* i_mirror) { m_brotherMagicMirror = i_mirror->GetPtr(); }

	void setBlingInterval(float i_interval) { m_blingInterval = i_interval; }
...
private:
	...
	RtWeakPtr<GridItemMagicMirror2> m_brotherMagicMirror;
	pvztime_t m_disappeartime;
	pvztime_t m_blingTime;
	float m_blingInterval;
};

class GridItemMagicMirrorRig2 : public PopAnimRig
{
public:
	RT_CLASS_DEFINE(GridItemMagicMirrorRig2, PopAnimRig, RtClass);

	void playAppearAnim(PopAnimRig::AnimStoppedReflectionDelegate i_onAnimStopped);
	void playWorkingAnim();
	void playWorkingBlingAnim(PopAnimRig::AnimStoppedReflectionDelegate i_onAnimStopped);
	...
	const std::string getWorkingBlingAnimLabel();
...
class MagicMirrorTeleportationArraySubSystem2 : public GameSubSystem
{
public:
	RT_CLASS_DEFINE(MagicMirrorTeleportationArraySubSystem2, GameSubSystem, RtClass);
...
#endif /* PLANTSVERSUSZOMBIES2_PVZ2_GRIDITMEMAGICMIRROR2_H_ */
