...
#if defined(EA_PROCESSOR_POWERPC) || defined(EA_PLATFORM_XENON) || defined(EA_PLATFORM_PS3) || defined(EA_PLATFORM_PS3_SPU)
    #define EA_THREAD_ATOMIC_IMPLEMENTED

    #if defined(EA_PLATFORM_XENON)
        #if EATHREAD_XTL_H_ENABLED  // For backward compatibility.
        ...
        typedef long *LPLONG;
        typedef long LONG;
        typedef int64_t *LPLONGLONG;
        typedef int64_t LONGLONG;
        extern "C" long     __stdcall _InterlockedIncrement(long volatile* Addend);
        extern "C" int64_t  __stdcall _InterlockedIncrement64(int64_t volatile* Addend);
        extern "C" long     __stdcall _InterlockedDecrement(long volatile* Addend);
        extern "C" int64_t  __stdcall _InterlockedDecrement64(int64_t volatile* Addend);
        ...
        extern "C" int64_t  __stdcall _InterlockedExchange64(int64_t volatile* Target, int64_t Value);
        extern "C" long     __stdcall _InterlockedExchangeAdd(long volatile* Addend, long Value);
        extern "C" int64_t  __stdcall _InterlockedExchangeAdd64(int64_t volatile* Addend, int64_t Value);
        extern "C" int64_t  __stdcall _InterlockedCompareExchange64(int64_t volatile* Dest, int64_t Exchange, int64_t Comp);
        ...
        #define EA_THREAD_ATOMIC_LLR_SUPPORTED (1)
    #else
            ...
            */
            ...
            protected:
                ...
                AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::GetValue() const
                    { return (ValueType)_InterlockedExchangeAdd((LPLONG)&mValue, 0); }
                ...
                AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::SetValue(ValueType n)
                    { return (ValueType)_InterlockedExchange((LPLONG)&mValue, (LONG)n); }
                ...
                bool AtomicInt<int32_t>::SetValueConditional(ValueType n, ValueType condition)
                    { return ((ValueType)_InterlockedCompareExchange((LPLONG)&mValue, (LONG)n, (LONG)condition) == condition); }
                ...
                AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::Increment()
                    { return (ValueType)_InterlockedIncrement((LPLONG)&mValue); }
                ...
                AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::Decrement()
                    { return (ValueType)_InterlockedDecrement((LPLONG)&mValue); }
                ...
                AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::Add(ValueType n)
                    { return ((ValueType)_InterlockedExchangeAdd((LPLONG)&mValue, (LONG)n) + n); }
                ...
                AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::GetValue() const
                    { return (ValueType)_InterlockedExchangeAdd64((LONGLONG*)&mValue, 0); }
                ...
                AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::SetValue(ValueType n)
                    { return (ValueType)_InterlockedExchange64((LONGLONG*)&mValue, (LONGLONG)n); }
                ...
                bool AtomicInt<int64_t>::SetValueConditional(ValueType n, ValueType condition)
                    { return ((ValueType)_InterlockedCompareExchange64((LONGLONG*)&mValue, (LONGLONG)n, (LONGLONG)condition) == condition); }
                ...
                AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::Increment()
                    { return (ValueType)_InterlockedIncrement64((LONGLONG*)&mValue); }
                ...
                AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::Decrement()
                    { return (ValueType)_InterlockedDecrement64((LONGLONG*)&mValue); }
                ...
                AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::Add(ValueType n)
                    { return ((ValueType)_InterlockedExchangeAdd64((LONGLONG*)&mValue, (LONGLONG)n) + n); }
                ...
                // The CELLATOMIC macros handle the scratch buffer required on SPU. Most of the time, a single
                // global buffer can be shared, because only one atomic operation is going on at a time. But if
                // you have multiple fibers, and the fibers are manually reserving lines and doing conditional
                // stores, each fiber needs a unique scratch buffer, so that it isn't possible for a reservation
                // made by one fiber to be accidentally committed by another. That is handled as a special case by
                // Reserve() and StoreConditionalReserved().

                #if defined( EA_PLATFORM_PS3_SPU )

                    struct AtomicScratch { static uint8_t sBuffer[128] __attribute__(( aligned( 128 ) )); };

                    #define CELLATOMIC_ADDR32( _EA )    (uint32_t*) AtomicScratch::sBuffer, (uint64_t) (_EA)
                    #define CELLATOMIC_ADDR64( _EA )    (uint64_t*) AtomicScratch::sBuffer, (uint64_t) (_EA)

                #else

                    #define CELLATOMIC_ADDR32( _EA )    (uint32_t*) (_EA)
                    #define CELLATOMIC_ADDR64( _EA )    (uint64_t*) (_EA)
                ...
                template <> inline
                AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::GetValue() const
                    { return (ValueType) cellAtomicNop32(CELLATOMIC_ADDR32(&mValue)); }
                ...
                template <> inline
                AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::SetValue(ValueType n)
                    { return (ValueType) cellAtomicStore32(CELLATOMIC_ADDR32(&mValue), (uint32_t)n); }

                template <> inline
                AtomicInt<uint32_t>::ValueType AtomicInt<uint32_t>::SetValue(ValueType n)
                    { return (ValueType) cellAtomicStore32(CELLATOMIC_ADDR32(&mValue), (ValueType)n); }

                template <> inline
                bool AtomicInt<int32_t>::SetValueConditional(ValueType n, ValueType condition)
                    { return ((ValueType) cellAtomicCompareAndSwap32(CELLATOMIC_ADDR32(&mValue), (uint32_t)condition, (uint32_t)n) == condition); }

                template <> inline
                bool AtomicInt<uint32_t>::SetValueConditional(ValueType n, ValueType condition)
                    { return ((ValueType) cellAtomicCompareAndSwap32(CELLATOMIC_ADDR32(&mValue), (ValueType)condition, (ValueType)n) == condition); }

                template <> inline
                AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::Increment()
                    { return (ValueType) cellAtomicIncr32(CELLATOMIC_ADDR32(&mValue)) + 1; }
                ...
                template <> inline
                AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::Decrement()
                    { return (ValueType) cellAtomicDecr32(CELLATOMIC_ADDR32(&mValue)) - 1; }
                ...
                template <> inline
                AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::Add(ValueType n)
                    { return (ValueType) cellAtomicAdd32(CELLATOMIC_ADDR32(&mValue), (uint32_t)n) + n; }

                template <> inline
                AtomicInt<uint32_t>::ValueType AtomicInt<uint32_t>::Add(ValueType n)
                    { return (ValueType) cellAtomicAdd32(CELLATOMIC_ADDR32(&mValue), (ValueType)n) + n; }

                #if defined( EA_PLATFORM_PS3_SPU )
                    template <> inline
                    AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::Reserve(void* spuScratch)
                        { return (ValueType) cellAtomicLockLine32((uint32_t*) spuScratch, (uint64_t) &mValue); }

                    template <> inline
                    AtomicInt<uint32_t>::ValueType AtomicInt<uint32_t>::Reserve(void* spuScratch)
                        { return (ValueType) cellAtomicLockLine32((uint32_t*) spuScratch, (uint64_t) &mValue); }

                    template <> inline
                    bool AtomicInt<int32_t>::StoreConditionalReserved(AtomicInt<int32_t>::ValueType n, void* spuScratch)
                        { return (cellAtomicStoreConditional32((uint32_t*) spuScratch, (uint64_t) &mValue, n) == 0); }

                    template <> inline
                    bool AtomicInt<uint32_t>::StoreConditionalReserved(AtomicInt<uint32_t>::ValueType n, void* spuScratch)
                        { return (cellAtomicStoreConditional32((uint32_t*) spuScratch, (uint64_t) &mValue, n) == 0); }
                #else
                    template <> inline
                    AtomicInt<int32_t>::ValueType AtomicInt<int32_t>::Reserve(void*)
                        { return (ValueType) cellAtomicLockLine32(CELLATOMIC_ADDR32(&mValue)); }

                    template <> inline
                    AtomicInt<uint32_t>::ValueType AtomicInt<uint32_t>::Reserve(void*)
                        { return (ValueType) cellAtomicLockLine32(CELLATOMIC_ADDR32(&mValue)); }

                    template <> inline
                    bool AtomicInt<int32_t>::StoreConditionalReserved(AtomicInt<int32_t>::ValueType n, void*)
                        { return (cellAtomicStoreConditional32(CELLATOMIC_ADDR32(&mValue), n) == 0); }

                    template <> inline
                    bool AtomicInt<uint32_t>::StoreConditionalReserved(AtomicInt<uint32_t>::ValueType n, void*)
                        { return (cellAtomicStoreConditional32(CELLATOMIC_ADDR32(&mValue), n) == 0); }
                #endif
                ...
                template <> inline
                AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::GetValue() const
                    { return (ValueType)  cellAtomicNop64(CELLATOMIC_ADDR64(&mValue)); }
                ...
                template <> inline
                AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::SetValue(ValueType n)
                    { return (ValueType)  cellAtomicStore64(CELLATOMIC_ADDR64(&mValue), (uint64_t)n); }

                template <> inline
                AtomicInt<uint64_t>::ValueType AtomicInt<uint64_t>::SetValue(ValueType n)
                    { return (ValueType)  cellAtomicStore64(CELLATOMIC_ADDR64(&mValue), (ValueType)n); }

                template <> inline
                bool AtomicInt<int64_t>::SetValueConditional(ValueType n, ValueType condition)
                    { return ((ValueType) cellAtomicCompareAndSwap64(CELLATOMIC_ADDR64(&mValue), (uint64_t)condition, (uint64_t)n) == condition); }

                template <> inline
                bool AtomicInt<uint64_t>::SetValueConditional(ValueType n, ValueType condition)
                    { return ((ValueType) cellAtomicCompareAndSwap64(CELLATOMIC_ADDR64(&mValue), (ValueType)condition, (ValueType)n) == condition); }

                template <> inline
                AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::Increment()
                    { return (ValueType)  cellAtomicIncr64(CELLATOMIC_ADDR64(&mValue)) + 1; }
                ...
                template <> inline
                AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::Decrement()
                    { return (ValueType)  cellAtomicDecr64(CELLATOMIC_ADDR64(&mValue)) - 1; }
                ...
                template <> inline
                AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::Add(ValueType n)
                    { return (ValueType)  cellAtomicAdd64(CELLATOMIC_ADDR64(&mValue), (uint64_t)n) + n; }

                template <> inline
                AtomicInt<uint64_t>::ValueType AtomicInt<uint64_t>::Add(ValueType n)
                    { return (ValueType)  cellAtomicAdd64(CELLATOMIC_ADDR64(&mValue), (ValueType)n) + n; }

                #if defined( EA_PLATFORM_PS3_SPU )
                    template <> inline
                    AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::Reserve(void* spuScratch)
                        { return (ValueType) cellAtomicLockLine64((uint64_t*) spuScratch, (uint64_t) &mValue); }

                    template <> inline
                    AtomicInt<uint64_t>::ValueType AtomicInt<uint64_t>::Reserve(void* spuScratch)
                        { return (ValueType) cellAtomicLockLine64((uint64_t*) spuScratch, (uint64_t) &mValue); }

                    template <> inline
                    bool AtomicInt<int64_t>::StoreConditionalReserved(AtomicInt<int64_t>::ValueType n, void* spuScratch)
                        { return (cellAtomicStoreConditional64((uint64_t*) spuScratch, (uint64_t) &mValue, n) == 0); }

                    template <> inline
                    bool AtomicInt<uint64_t>::StoreConditionalReserved(AtomicInt<uint64_t>::ValueType n, void* spuScratch)
                        { return (cellAtomicStoreConditional64((uint64_t*) spuScratch, (uint64_t) &mValue, n) == 0); }
                #else
                    template <> inline
                    AtomicInt<int64_t>::ValueType AtomicInt<int64_t>::Reserve(void*)
                        { return (ValueType) cellAtomicLockLine64(CELLATOMIC_ADDR64(&mValue)); }

                    template <> inline
                    AtomicInt<uint64_t>::ValueType AtomicInt<uint64_t>::Reserve(void*)
                        { return (ValueType) cellAtomicLockLine64(CELLATOMIC_ADDR64(&mValue)); }

                    template <> inline
                    bool AtomicInt<int64_t>::StoreConditionalReserved(AtomicInt<int64_t>::ValueType n, void*)
                        { return (cellAtomicStoreConditional64(CELLATOMIC_ADDR64(&mValue), n) == 0); }

                    template <> inline
                    bool AtomicInt<uint64_t>::StoreConditionalReserved(AtomicInt<uint64_t>::ValueType n, void*)
                        { return (cellAtomicStoreConditional64(CELLATOMIC_ADDR64(&mValue), n) == 0); }
                #endif


            #else // Generic PowerPC: Macintosh OSX, etc.
