#ifndef __SEXYAPPFRAMEWORK_PS3_PLATFORM_H__
#define __SEXYAPPFRAMEWORK_PS3_PLATFORM_H__
...
//This is a nasty workaround... for some reason the compiler isn't finding the _Vacopy declaration
//even though it's plainly next to the va_copy #define in yvals.h...  
//I don't know why it's not working there, but this fixes it for now. :\
...
static inline unsigned long long bswap_64(unsigned long long x) {
	return (((unsigned long long)bswap_32((int)(x&0xffffffffull)))<<32) |
	(bswap_32((int)(x>>32)));
}
...
#define _abs64(x) abs(x)

namespace Sexy
{

inline signed int AtomicIncrement(signed int* ioValue) {
	return (signed int)cellAtomicIncr32((uint32_t*)ioValue) + 1; }

inline signed int AtomicDecrement(signed int* ioValue) {
	return (signed int)cellAtomicDecr32((uint32_t*)ioValue) - 1; }

} // namespace Sexy

#endif // __SEXYAPPFRAMEWORK_PS3_PLATFORM_H__
