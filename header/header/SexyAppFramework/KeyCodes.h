...
enum KeyCode
{
    ...
    KEYCODE_OTHER_PLATFORM_BACK_QUOTE = 0xC0,   // From changelist 62830 - We have no idea what this was to support.
	KEYCODE_ASCIIEND2	  = 0xE0,
	
	//used on Macs. May be needed in some Widget code to specifically tailor to macs
	KEYCODE_COMMAND		  = 0xF0,

	//used on Android. Hardware back button.
	KEYCODE_GOBACK		  = 0xF1,
};

KeyCode	GetKeyCodeFromName(const std::string& theKeyName);
const std::string GetKeyNameFromCode(const KeyCode& theKeyCode);
SexyChar GetCharFromKeyCode(const KeyCode& theKeyCode);
